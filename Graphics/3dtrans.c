...
int x=50,y=60;
int x1=50,x2=60,x3=60,x4=50,y1=50,y2=50,y3=60,y4=60,x5=57,y5=67,x6=67,y6=67,x7=67,y7=57;
//Cuboid shape from 
	linee(x1,y1,x2,y2);
	linee(x1,y1,x4,y4);
	linee(x3,y3,x2,y2);
...
//Translation factor
int tx=15,ty=25;
x1+=tx; y1+=ty;x2+=tx; y2+=ty;x3+=tx; y3+=ty;x4+=tx; y4+=ty;x5+=tx; y5+=ty;x6+=tx; y6+=ty;x7+=tx; y7+=ty;
...
y4=(y0+((a-x0)*sin(theta)+(b-y0)*cos(theta)));
 a=x5,b=y5;
x5=(x0+((a-x0)*cos(theta)-(b-y0)*sin(theta)));
y5=(y0+((a-x0)*sin(theta)+(b-y0)*cos(theta)));
 a=x6,b=y6;
x6=(x0+((a-x0)*cos(theta)-(b-y0)*sin(theta)));
y6=(y0+((a-x0)*sin(theta)+(b-y0)*cos(theta)));
 a=x7,b=y7;
x7=(x0+((a-x0)*cos(theta)-(b-y0)*sin(theta)));
y7=(y0+((a-x0)*sin(theta)+(b-y0)*cos(theta)));
...
//Scaling factor
float sx=1.2,sy=1.2;
x1*=sx;y1*=sy; x2*=sx;y2*=sy; x3*=sx;y3*=sy; x4*=sx;y4*=sy; x5*=tx; y5*=ty;x6*=tx; y6*=ty;x7*=tx; y7*=ty;
